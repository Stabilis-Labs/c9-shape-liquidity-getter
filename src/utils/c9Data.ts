...
export async function getC9BinData(
  ...
  try {
    ...
    if (stateVersion !== undefined) {
      const currentStateVersion = await api.status.getCurrent().then(response => response.ledger_state.state_version);
      if (stateVersion > currentStateVersion) {
        throw DataError.stateVersionTooHigh(stateVersion);
      }
    }

    const ledgerState: LedgerStateSelector | undefined = stateVersion ? { state_version: stateVersion } : undefined;

    // 1. Get component data
    const componentData = await getComponentData(api, componentAddress, stateVersion);
    if (!componentData) {
    ...
    if (!binMapField?.value || !nftManagerField?.value || 
        !activeXField?.value || !activeYField?.value || !activeTotalClaimField?.value) {
      console.error('Required fields not found in component state');
      return null;
    }

    // Extract current tick if available
    const currentTickField = tickIndexField?.fields?.find(f => f.field_name === 'current')
      ?.fields?.[0]?.fields?.[0]?.value;
    const currentTick = currentTickField ? parseInt(currentTickField) : undefined;

    const kvStoreAddress = binMapField.value;

    // 3. Get all keys from the KV store
    const keys = await getAllKeyValueStoreKeys(api, kvStoreAddress, ledgerState);
    ...
    for (const entry of rawBinMapData) {
      ...
      if (tick && valueJson?.fields) {
        ...
        const amount = fields.find((f: ScryptoField) => f.field_name === 'amount')?.value || '0';
        const totalClaim = fields.find((f: ScryptoField) => f.field_name === 'total_claim')?.value || '0';
...
export async function calculateRedemptionValue(
  ...
  try {
    ...
    // 2. Get NFT data
    const api = getGatewayApi();
    const nftDataResponse = await getNFTData(api, c9Data.nftAddress, nftId, stateVersion);
    ...
    for (const entry of liquidityClaimsField.entries) {
      ...
      if (tick < c9Data.currentTick) {
        ...
        if (bin) {
          ...
          amount_y = new Decimal(amount_y).plus(share.times(bin.amount)).toString();
        }
        ...
        if (bin) {
          const share = new Decimal(claimAmount).div(bin.total_claim);
          amount_x = new Decimal(amount_x).plus(share.times(bin.amount)).toString();
        }
      } else {
        // Active bin - both X and Y tokens
        const liquidityShare = new Decimal(claimAmount).div(c9Data.active_total_claim);
        amount_x = new Decimal(amount_x).plus(new Decimal(c9Data.active_x).times(liquidityShare)).toString();
        amount_y = new Decimal(amount_y).plus(new Decimal(c9Data.active_y).times(liquidityShare)).toString();
      }
    }
